...
def _retime_day(activities: list[dict[str, Any]], city: str, sandbox, plan: Plan, start_pos: str = "") -> str:
    ...
    for act in activities:
        atype = act.get("type", "")
        duration = _activity_duration(act)

        if atype in ("airplane", "train"):
            if current_pos:
                try:
                    start = max_time(current_time, act.get("start_time", current_time))
                    act["start_time"] = start
                    act["end_time"] = add_minutes(start, duration, allow_overflow=True)
                    current_time = add_minutes(act.get("end_time", current_time), 20, allow_overflow=True)
                except RuntimeError:
                    pass  # 保留原时间
            current_pos = str(act.get("end", current_pos))
            continue
        ...
        if position:
            try:
                ...
                # 时间溢出：保留 plan_builder 已设定时间，跳过此 activity 的重排
                current_pos = position
        else:
            try:
                act["start_time"] = current_time
                act["end_time"] = add_minutes(current_time, duration)
                current_time = add_minutes(act["end_time"], 10)
            except RuntimeError:
                pass  # 保留原时间
    return current_pos
...
def _initial_day_time(activities: list[dict[str, Any]]) -> str:
    ...
    if activities[0].get("type") in {"airplane", "train"}:
        return activities[0].get("start_time", "08:00")
    return "09:00"

...
结果写入（--official-results）: ChinaTravel/results/TPCAgent_TPCLLM_en/{uid}.json（--lang en 时）
"""
...
from scripts.eval_utils import effective_method_name, load_official_split
...
def load_split_queries(
    ...
    *,
    ...
    # 仍未找到：尝试 ChinaTravel 官方 split（含 HuggingFace / training data 回退）
    if not query_ids and ct_root:
        ct_split = (
            ct_root / "chinatravel" / "evaluation" / "default_splits" / f"{splits}.txt"
        )
        local_split = PROJECT_ROOT / "data" / "splits" / f"{splits}.txt"
        if ct_split.exists() or local_split.exists():
            ids, data = load_official_split(
                splits, limit=limit or 0, ct_root=ct_root, lang=lang
            )
            if ids:
                return ids, data
...
def main() -> None:
    ...
    query_ids, query_data = load_split_queries(
        args.splits,
        single_uid=args.index,
        limit=args.limit,
        official=args.official_results,
        lang=args.lang,
    )
    if args.index:
    ...
    results_method = effective_method_name(method, args.lang)

    # 输出目录：--official-results → ChinaTravel/results/；否则 config → 默认
    ct_root = _resolve_chinatravel_root()
    if args.official_results and ct_root:
        results_dir = ct_root / "results" / results_method
    ...
    if args.official_results and ct_root:
        eval_cmd = (
            ...
            f"--splits {args.splits} --method {method} --lang {args.lang}"
        )
        print(f"\n官方 eval 就绪，运行:\n  {eval_cmd}")
        if results_method != method:
            print(f"  实际读取目录: ChinaTravel/results/{results_method}/")
